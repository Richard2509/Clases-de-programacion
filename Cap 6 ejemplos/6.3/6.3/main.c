...
/*Universidad.
El programa,a l recibir informacion sobre el numeo de alumnos que han ingresado a sus ocho diferentes carreras en los dos semestres lectivos de los ultimos cinco anos, obtiene informacion util para el departamento de escolar.*/
...
/* La funcion Lectura se utiliza para leer un arreglo tridimensional d tipo entero de FI filas, CO columnas y PR profundidad. Observa que al ser tridimensional se necesitan tres ciclos para recorrer el arreglo. */
{
    int K, I, J;
    for (K=0; K<PR; K++)
        for (I=0; I<FI; I++)
...
/*Esta funcion se utiliza para determinar la carrera que recibio el mayor numero de alumnos el ultimo a;o. Observa que no se utiliza un ciclo para los planos de la profundidad, ya que es un dato (PR) */
{
...
/* Esta funcion se utiliza para determinar el a;o en el que la carrera Ingenierioa en computacion recibio el mayor numero de alumnos. Observa que no se utiliza un ciclo para trabajar con las filas, ya que es un dato (FI). */
{
