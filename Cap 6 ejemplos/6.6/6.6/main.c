...
/* Intercambia.
El programa intercambia las columnas de un arreglo bidimensional. Los elementos de la primera columna se intercamvian con los de la ultima, los de la segunda con los de la penultima, y asi sucesivamente. */
...
/* Esta funcion se utiliza para intercambiar las columnas del arreglo bidimensional. Observa que el indice correspondiente a las columnas solo se mueve hasta la mitad del arreglo. */
{
    int I, J;
    float AUX;
    /* Onserva que en esta funcion el indice I se utiliza para las columnas, y el indice J para las filas. */
    for (I=0; I < (C / 2); I++)
        for (J=0; J < F; J++)
    {
        AUX = A[J][I];
...
/* La funcion Imprime se utiliza para escribir un arreglo bidimensional de tipo real de F filas y C columnas. */
{
    ...
    for (I=0; I < F; I++)
        for (J=0; J < C; J++)
        printf("\nElementos %d %d: %5.2f", I+1, J+1, A[I][J]);
}
